...
export const load = (async ({ params }) => {
    ...
    let wasmHash = instance?.entries[0].val.contractData().val().instance().executable().wasmHash().toString('hex')
    ...
    instance?.entries[0].val.contractData().val().instance().storage()?.forEach((v) => {
